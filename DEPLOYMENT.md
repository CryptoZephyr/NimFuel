...
- Runtime: native Node
- Region: Frankfurt
- Source: public `CryptoZephyr/NimFuel`, branch `main`
...
The current backend deployment is live from commit `aafeb69`. Hosted `/health` returns `status=pass` and `ready=true`, and `GET /v1/orders?evmAddress=...` returns the Stage 2 history contract. The next hosted check is a clean user-authorized Nimiq Pay flow, including relay and receipt verification.
